...
const UserListTable: React.FC<UserListTableProps> = ({
  ...
  const [blockCognitoUserDialogOpen, setBlockCognitoUserDialogOpen] =
    useState<boolean>(false);
  const [cognitoUserToBlock, setCognitoUserToBlock] = useState<
  ...
  const defaultColumns = React.useMemo<ColumnDef<User>[]>(
    () => [
      ...
      },
      {
        ...
        accessorFn: (row) => {
          const cognitoUser = cognitoUserList.find(
            (cognitoUserItem) => cognitoUserItem?.email === row.email,
          );
          ...
          return cognitoUser.groups
            .map((group) => {
              const groupOption = USER_GROUPS_OPTIONS.find(
      ...
      },
      {
        ...
        accessorFn: (row) =>
          row.createdAt
            ? utils.dates.getDateInGermanFromAWSDateFormat(row.createdAt)
            : "",
        cell: (info) => info.getValue(),
        footer: (props) => props.column.id,
      },
