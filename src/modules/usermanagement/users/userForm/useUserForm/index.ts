...
import { USER_GROUPS_OPTIONS } from "modules/usermanagement/roles/api";
...
const useUserForm = (
  ...
  const [firstName, setFirstName] = usePersistedState<string>(
    "user_firstName",
    "",
    formIntent,
  );
  const [lastName, setLastName] = usePersistedState<string>(
    "user_lastName",
    "",
    formIntent,
  );
  const [salutation, setSalutation] = usePersistedState<Salutation | null>(
    "user_salutation",
    null,
    formIntent,
  );
  const [email, setEmail] = usePersistedState<string>(
    "user_email",
    "",
    formIntent,
  );
  const [phone, setPhone] = usePersistedState<string>(
    "user_phone",
    "",
    formIntent,
  );
  ...
  const [avatar, setAvatar] = usePersistedState<S3Resource | null>(
    "user_avatar",
    null,
    formIntent,
  );
  ...
  const [tableHeaderColor, setTableHeaderColor] =
    useState<TableHeaderColor | null>(null);
  ...
  const [uploading, setUploading] = usePersistedState<boolean>(
    "user_uploading",
    false,
    formIntent,
  );
  const [uploadProgress, setUploadProgress] = usePersistedState<number>(
    "user_uploadProgress",
    0,
    formIntent,
  );
  ...
  useEffect(() => {
    ...
    const roles = USER_GROUPS_OPTIONS.filter((userGroupOption) =>
      cognitoUser.groups.some(
        (group) => group.GroupName === userGroupOption.value,
      ),
    );
  ...
  useEffect(() => {
    ...
    const unitTypeOption = utils.constants.WEIGHT_OPTIONS.find(
      (unitTypeOptionItem) =>
        unitTypeOptionItem.value === user.userSettings.unitType,
    );
    if (unitTypeOption) {
