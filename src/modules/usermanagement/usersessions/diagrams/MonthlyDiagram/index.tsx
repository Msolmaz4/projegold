...
const MonthlyDiagram: React.FC = () => {
  ...
  const diagramData: DailyAggregation[] = useMemo(() => {
    const dailyAggregation: DailyAggregation[] = Array.from(
  ...
  const totalMinutes = useMemo(
    () => diagramData.reduce((acc, item) => acc + item.totalDurationMin, 0),
    [diagramData],
  );
  ...
  return (
    <div className={classes.card}>
      <Grid2 container direction="row" spacing={2} alignItems="center">
        <CustomButton
          text={
            getMonthName(dayjs(startDate).subtract(1, "month").month() + 1) +
            " " +
            dayjs(startDate).subtract(1, "month").year()
          }
        ...
        <Typography variant="h5" color="textPrimary">
          {getMonthName(dayjs(startDate).month() + 1) +
            " " +
            dayjs(startDate).year()}
        </Typography>
        <CustomButton
          text={
            getMonthName(dayjs(startDate).add(1, "month").month() + 1) +
            " " +
            dayjs(startDate).add(1, "month").year()
          }
