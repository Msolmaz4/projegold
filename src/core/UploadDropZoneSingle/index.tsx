...
interface UploadDropZoneSingleProps extends CustomFormLabelProps {
...
const UploadDropZoneSingleComponent: React.ForwardRefRenderFunction<
  FieldHandles,
  UploadDropZoneSingleProps
> = (
  {
    dragActiveText,
    dragInactiveText,
  ...
  const [saveInfo, setSaveInfo] = useState<boolean>(
    showSaveInfo && image !== null && !image.fromDatabase,
  );

  useEffect(() => {
    setSaveInfo(showSaveInfo && image !== null && !image.fromDatabase);
  }, [image, showSaveInfo]);
...
export default React.forwardRef(UploadDropZoneSingleComponent);
