...
const Breadcrumbs: React.FC = () => {
  ...
  const getPathOfRoute = (route: AppRoute) => {
    let path = route.path
      ...
      .replace(":orderID", dataContext.order?.id ?? "");

    if (route.isWithParam) {
      path = path.replace(
        getRouteParamName(route.path),
        dataContext.currentData?.id ?? "",
      );
    }
    return path;
  };
  ...
  const getNameOfRouteWithParam = (route: AppRoute) => {
    ...
    if (routeLastParamName === ":customerID" && !checkRoute(pathname, route)) {
      return dataContext.customer?.kurzName;
    } else if (
      routeLastParamName === ":orderID" &&
      ...
      <div ref={breadcrumbsContentRef} className={classes.breadcrumbsContent}>
        ...
        {breadcrumbs.map((route, i) =>
          i + 1 === breadcrumbs.length ? (
            route.isWithParam ? (
              dataContext.isLoadingCurrentData ||
              dataContext.isLoadingStateEntities ? (
